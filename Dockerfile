...
#
# Приложение написано под Cloudflare Workers + D1. Чтобы не переписывать
# слой доступа к данным, worker-бандл здесь исполняется в workerd через
# miniflare — тот же рантайм, что и в Cloudflare, с локальным D1 (SQLite)
# на диске. Статика раздаётся nginx: он отдаёт файл из dist/client, если тот
# есть, а всё остальное (включая "/") проксирует в SSR-worker — ровно та же
# схема "asset-first", что описана в wrangler.jsonc.
