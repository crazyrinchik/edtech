        ...
        {homework.length === 0 ? (
          <p className="sov-kid__free">
            {data.hasTutor
              ...
              : "Задания сейчас нет — выбирай тему сам или загляни в тренажёры."}
          </p>
        ) : null}
        ...
        {homework.map((hw) => (
          <section key={hw.id} className="sov-homework" data-status={hw.status}>
            <div className="sov-homework__head">
              <strong>
                {hw.status === "done" ? `${hw.title} — сделано!` : hw.title}
                {/* Кто задал. Домашку выдаёт и наставник, и родители, и
                    когда в списке лежат обе, ребёнку важно, кому он потом
                    покажет тетрадь. С одним взрослым подпись лишняя, но
                    сервер не знает, сколько их, — она стоит всегда. */}
                <em className="sov-homework__by">
                  {hw.author === "parent" ? "от родителей" : "от наставника"}
                </em>
              </strong>
...
/**
 * Своё задание от взрослого — наставника или родителей: текст, файл и
 * поле ответа.
 *
 * Проверяет его человек, а не платформа, поэтому здесь нет «верно» и
 * «неверно» — есть отправленный ответ и оценка, когда задание посмотрят.
 */
function CustomItem({
  item,
  onDone,
    ...
    <div id={`zadanie-${item.id}`} className="sov-homework__custom" data-done={!!item.grade}>
      ...
      {item.grade ? (
        ...
        <>
          ...
          >
            {item.submittedAt ? "Отправить ещё раз" : "Отправить на проверку"}
          </button>
          {item.submittedAt ? (
