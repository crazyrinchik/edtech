...
function TablePage() {
  ...
  /* Салют на экране итога: из next() его звать нельзя — небо принадлежит
     стадии упражнения и исчезает вместе с ней. */
  useEffect(() => {
            ...
            {verdict ? (
              <div className="sov-feedback" data-kind={verdict.ok ? "right" : "wrong"}>
                <div>
                  ...
                  <span>
                    {verdict.ok ? "Идём дальше." : `Правильный ответ: ${verdict.answer}.`}
                  </span>
                </div>
              </div>
            ) : null}
