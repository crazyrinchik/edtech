...
type AppEnv = {
   ...
   * Статический ключ доступа сервисного аккаунта Yandex Cloud с ролью
   * postbox.sender — идентификатор и секрет — и подтверждённый в Postbox
   * адрес отправителя. Без них форма восстановления не притворяется
   * рабочей: она честно показывает адрес поддержки.
   */
  POSTBOX_KEY_ID?: string;
  POSTBOX_SECRET_KEY?: string;
  MAIL_FROM?: string;
