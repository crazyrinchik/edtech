...
export const finishTopic = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // В мессенджер уходит только само событие — ни имени, ни темы, ни
    // процентов. Telegram и MAX это чужие серверы, и пункт 6.3 политики
    // обещает, что персональных данных туда не уходит; имя ребёнка ими
    // является, поэтому его здесь нет и быть не должно. Обратная сторона
    // известна и принята: родитель, у которого занимаются двое, по самому
    // уведомлению не поймёт, о ком речь, — за этим он идёт в кабинет.
    // Род ребёнка приложение не спрашивает, поэтому в сообщении нет
    // глаголов прошедшего времени: «окончена проверочная», а не
    // «позанимался/позанималась».
    if (data.mode === "check") {
      await notifyParent(user.id, "Совёнок: окончена проверочная. Подробности — в кабинете.");
    }
...
export const deleteAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Учебный след взрослого уходит вместе с ним — на всех учеников,
    ...
    // остаются семье. tutor_id здесь значит «кто задал»: домашку выдаёт и
    // родитель (tutor.functions.ts), и его задания уходят тем же путём —
    // хотя его дети к этому моменту уже вычищены purgeChildData выше.
    //
