...
/*
 * Судоку: поле на четыре клетки, две из них заполнены.
 *
 * Первым рисунком здесь была сетка Шульте с закрашенной серединой — и в
 * ленте на витрине, где оба тренажёра стоят рядом, они не различались
 * вовсе: те же девять клеток, чуть плотнее в центре. Дело не в закраске, а
 * в том, что силуэт совпадал целиком.
 *
 * Поэтому у судоку клеток четыре, а не девять: 2 × 2 — это и есть квадрат
 * поля 4×4, с которого тренажёр открывается. Две клетки закрашены, две
 * пусты: у Шульте числа ищут в готовой таблице, здесь половину вписывают.
 */
export function SudokuIcon({ size = 24 }: { size?: number }) {
  return icon(
    <>
      <rect x="3" y="3" width="18" height="18" rx="2.5" />
      <path d="M12 3v18M3 12h18" />
      <rect x="5.1" y="5.1" width="4.8" height="4.8" rx="1" fill="currentColor" stroke="none" />
      <rect x="14.1" y="14.1" width="4.8" height="4.8" rx="1" fill="currentColor" stroke="none" />
    </>,
    size,
  );
}
