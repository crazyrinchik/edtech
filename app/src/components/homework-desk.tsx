 ...
 * ученика, родитель — про ребёнка. Раздел с программой у них теперь один
 * и называется одинаково — «Повторение школьной программы»; раньше он был
 * только у репетитора, и подсказка про темы ссылалась на него лишь ему.
 */
const WORDS: Record<
  Audience,
  {
...
> = {
  tutor: {
    ...
    whoPl: "ученикам",
    topicsHint:
      "Здесь вся программа начальной школы — тот же список, что в разделе «Повторение школьной программы». Сначала класс ученика, соседние классы под отдельной строкой.",
    customIntro:
  ...
  parent: {
    ...
    whoPl: "детям",
    topicsHint:
      "Здесь вся программа начальной школы — тот же список, что в разделе «Повторение школьной программы». Сначала класс ребёнка, соседние классы под отдельной строкой.",
    customIntro:
    ...
    <>
      ...
      <form className="sov-panel sov-form" style={{ marginTop: 30 }} onSubmit={submit}>
        ...
        {/* Тренажёр задаётся со своими настройками.
            ...
            тренажёра, и раскрываются они только у отмеченного: шесть
