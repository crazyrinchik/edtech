...
export function AutoSpeakToggle({ onChange }: { onChange?: (on: boolean) => void }) {
  ...
  return (
    ...
    >
      {/* Переключатель, а не пилюля с ушком.

          Рядом на том же экране стоит кнопка «Слушать» — она озвучивает
          один вопрос. Обе выглядели одинаково: пилюля, ушко, короткая
          подпись. Ребёнок видел две кнопки со звуком и не мог понять, чем
          они отличаются, а отличаются они принципиально: одна делает
          действие сейчас, другая включает режим на всё занятие.

          Тумблер снимает вопрос формой: это настройка, у неё есть
          положение «включено» и «выключено». Ушко отсюда убрано и
          осталось только у кнопки действия. */}
      <span className="sov-switch" aria-hidden="true" />
      <span>Читать задания вслух</span>
    </button>
  );
}
